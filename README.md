...
- 一手ごとに手の良し悪し(最善手・好手・悪手など)をその場で判定し、対局後にプレイ全体をまとめて採点する。最善手を打てたときは火花エフェクトで強調する。
- **評価値表示** は任意で ON / OFF でき、対局しながら各手の評価値を確認できる。
- **答え合わせ** を ON にすると、自分の着手後に「最善手だったマス」を一瞬ハイライトで教えてくれる(評価値表示とは排他)。
...
- **盤面編集** で、途中の局面を自分で作ってから検討できる。石(黒 / 白 / 消す)を自由に配置し、手番を選んで「完了」すると、その局面から各手の評価値を計算する。
